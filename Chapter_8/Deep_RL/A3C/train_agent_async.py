...
def train_loop(process_idx, env, agent, steps, outdir, counter,
    ...
    try:
        ...
        env = env.env.env  # changed by FR
        env.set_agent(agent)  # changed by FR
        episode_r = 0
        global_t = 0
        local_t = 0
        global_episodes = 0
        ...
        while True:
            ...
            if done or reset or global_t >= steps or training_done.value:
                ...
                env.unwrapped.set_global_t(global_t)  # changed by FR: Note that we do not acquire a semaphore/lock on gloabl_t. Since we use it to decrease epsilon-greediness, the precise value is not too important
